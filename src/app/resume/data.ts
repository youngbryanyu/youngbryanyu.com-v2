...
export const experiences: Position[] = [
    {
        ...
        descriptionList: [
            "Developed and maintained several microservices and REST APIs essential to the core product function. Created and maintained internal developer documentation and external customer documentation.",
            ...
            "Built a CI/CD pipeline to automate unit testing, building/tagging/pushing Docker images, deploying updates with zero downtime rolling deployments, and managing infrastructure. Parallelized builds and leveraged an external cache with BuildKit for faster builds. Processed commits in batches, parallelized independent pipeline steps, and cached dependencies to improve build times.",
            "Created and managed Docker Swarm mode clusters for container orchestration across multiple on-premise machines in multiple environments. Utilized an Nginx reverse proxy on manager nodes to route external HTTPS requests to the ingress routing mesh. Integrated Cloudflare for DNS and SSL/TLS management.",
            "Developed idempotent Ansible playbooks for installing dependencies on host machines, configuring network settings, setting up Nginx, updating applications with zero downtime rolling deployments, and managing the Docker Swarm mode cluster. Ensured seamless cluster management by automating node operations (e.g. scaling up/down, promotion/demotion, labeling) based on inventory file changes.",
